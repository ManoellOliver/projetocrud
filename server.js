...
const app = express();
const uri = "mongodb+srv://dbUser:dbUser@cluster0.zmznqjt.mongodb.net/?retryWrites=true&w=majority";
...
app.post("/edit/:id", (req, res) => {
  ...
  var email = req.body.email; // Mudança de surname para email
  ...
  db.collection("crud").updateOne(
    { _id: ObjectId(id) },
    {
      $set: {
        name: name,
        email: email, // Mudança de surname para email
      },
    },
    (err, result) => {
      if (err) return res.send(err);
